...
def parse_work_command(text: str) -> WorkCommand:
    ...
    if "running" in t or "what's running" in t or "whats running" in t:
        return WorkCommand("inspect_runs", source="jarvis", autonomy="read_only", target_worker_id=target)
    if "resume" in t or "continue" in t:
        return WorkCommand("resume_run", source="jarvis", autonomy="start_if_unambiguous", target_worker_id=target)
    if "blocked" in t or "stalled" in t:
        return WorkCommand("inspect_blocked", source="jarvis", autonomy="read_only", target_worker_id=target)
    if "fix" in t or "address" in t or "handle" in t:
        kind = "pull_request" if "pr" in t or "review" in t or "comment" in t else "issue"
        return WorkCommand("start_selected_work", source=source, kind=kind, filters=filters, autonomy="start_if_unambiguous", start=True, target_worker_id=target)
    if "comment" in t and "pr" in t:
        return WorkCommand("inspect_pr_comments", source="github", kind="pull_request", filters=filters, autonomy="read_only", target_worker_id=target)
    if "get" in t or "take" in t or "pick up" in t or "start" in t or "work on" in t:
        kind = "ticket" if source == "linear" else "issue"
        return WorkCommand("start_next_work", source=source, kind=kind, filters=filters, autonomy="start_if_unambiguous", start=True, target_worker_id=target)
    if "check" in t or "show" in t or "list" in t or "summarize" in t:
        kind = "ticket" if source == "linear" else "issue"
        if "pr" in t or "review" in t:
            kind = "pull_request"
        return WorkCommand("inspect_work", source=source, kind=kind, filters=filters, autonomy="read_only", target_worker_id=target)
    return WorkCommand("direct_request", source="direct", filters={"text": text}, autonomy="read_only", target_worker_id=target)
