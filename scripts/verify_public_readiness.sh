  ...
  missing_patterns="$(
    {
      ...
      git -C "$ROOT_DIR" grep -q 'jarvis service sync brain api worker intercom' -- docs-site/index.html || echo "docs-site/index.html missing role sync command"
      ...
      git -C "$ROOT_DIR" grep -q -- '--expect-role brain --expect-role api --expect-role worker --expect-role intercom --expect-current-release --min-files 4 --output ~/Desktop/jarvis-bringup-evidence/jarvis-fleet-summary.json' -- docs-site/index.html || echo "docs-site/index.html missing current-release bring-up summary output command"
