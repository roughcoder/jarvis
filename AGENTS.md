...
src/jarvis/
  ...
  orchestration/       work-dispatch + Cockpit API tier (jarvis api): runs/sessions/projects store, executor, supervisor — talks to the worker over HTTP; reaches the brain in-process ONLY via brain/facade.py (the curated cross-tier contract, boundary-test enforced; one-directional — brain never imports it)
```
